...
if __name__ == "__main__":
    ...
    exit()
    # Create scenario
    scenario_id = args.scenario.split("/")[-2]
    data_local_path = "/home/kyber/argoverse/"
    scenario_dir = (
        pathlib.Path(args.output_dir.rstrip(args.output_dir[-1]))
        / f"{scenario_id}_agents_1"
    )
    scenario_dir.mkdir(exist_ok=True)
    filepath = pathlib.Path(f"{scenario_dir}/scenario.py")
    with filepath.open("w", encoding="utf-8") as f:
        f.write(
            f"""from pathlib import Path
